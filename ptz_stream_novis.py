...
_CAMERA_HEIGHT_WALL_PRIM = str(
    cfg.get("camera_height_wall_prim_path", "/World/Changjing_Jichu_2026_04_01/Changjing_AlgorithmVerification_2026_03/Model/Group5/Group7/AIShaoBing_Wall_Metal_2_26/Component431_26/Mesh228") or "/World/Changjing_Jichu_2026_04_01/Changjing_AlgorithmVerification_2026_03/Model/Group5/Group7/AIShaoBing_Wall_Metal_2_26/Component431_26/Mesh228"
).strip()
...
_scene_state = {
    "gondola_y": 0.0,    # 吊篮高度（gondola_y_min ~ max；写入 USD 时按 upAxis 落到高度轴）
    "workers":   2,      # 工人数量（0 / 1 / 2）
}
_scene_lock  = threading.Lock()
_scene_dirty = threading.Event()
_chosen_worker: int = 1   # workers=1 时固定使用工人 1（不随机）
...
def _set_prim_translate_height(stage, prim_path: str, height_val: float) -> None:
    """将 prim 的 xformOp:translate 在「高度轴」上的分量设为 height_val，其余分量不变。"""
    from pxr import Gf as _Gf
    prim = stage.GetPrimAtPath(prim_path)
    if not prim.IsValid():
        return
    ...
    if cur is not None:
        ...
        if idx == 2:
            attr.Set(_Gf.Vec3d(x, y, float(height_val)))
        else:
            attr.Set(_Gf.Vec3d(x, float(height_val), z))
    else:
        if idx == 2:
            attr.Set(_Gf.Vec3d(0.0, 0.0, float(height_val)))
        else:
            attr.Set(_Gf.Vec3d(0.0, float(height_val), 0.0))
...
def _set_resolved_workers_visible_count(stage, visible_count: int) -> int:
    """按当前解析到的 worker prim 运行时切换可见性。"""
    person_paths = [p for p in (_WORKER1_PRIM, _WORKER2_PRIM) if p]
    if not person_paths:
        return 0
    count = max(0, min(len(person_paths), int(visible_count)))
    visible_indices = set(range(count))
    if count == 1:
        visible_indices = {max(0, min(len(person_paths) - 1, _chosen_worker - 1))}
    for idx, p_path in enumerate(person_paths):
        _set_prim_visibility(stage, p_path, idx in visible_indices)
    return count


def _set_workers_visible_count(diaolan_info: dict, visible_count: int) -> int:
    """按固定人数显示工人，顺序稳定，避免重启后再次随机。"""
    person_paths = list(diaolan_info.get("persons") or [])
    count = max(0, min(len(person_paths), int(visible_count)))
    for idx, p_path in enumerate(person_paths):
        _set_prim_visibility(omni.usd.get_context().get_stage(), p_path, idx < count)
    return count
...
def _apply_scene_state(stage) -> None:
    ...
    _set_resolved_workers_visible_count(stage, workers)
...
def _describe_active_scene_branch(stage) -> dict:
    ...
    if model_prim and model_prim.IsValid():
        for child in model_prim.GetChildren():
            ...
            is_worker = c_path in (_WORKER1_PRIM, _WORKER2_PRIM)
    ...
    return {
        ...
        "worker_paths": [p for p in (_WORKER1_PRIM, _WORKER2_PRIM) if p],
...
def _apply_gondola_height_only(stage) -> None:
    """仅写吊篮根 prim 的高度轴；若 node1/node2 为兄弟则分别同步高度轴并保持各自初始轴向高度差。"""
    ...
    _set_prim_translate_height(stage, _GONDOLA_PRIM, gondola_y)
    node1_target_y = None
...
def _init_gondola_paths_camera_and_random_height_once(stage) -> None:
    ...
    # 2. Pick active and apply visibility
    forced_active_path = str(cfg.get("force_active_diaolan_path", "/World/DiaoLan_01") or "").strip()
    ...
    hidden_paths = [d["path"] for d in all_diaolans if d["path"] != active_diaolan["path"]]
    print(f"[diaolan-active] active={active_diaolan['path']} hidden={hidden_paths}")
    ...
    _last_gondola_init_group1_source = "diaolan_randomizer"
    ...
    # 4. Compute Changjing AABB first to bound the height
    aabb = compute_changjing_aabb(stage)
    
    # 3. Randomize workers and height with safety retry loop
    max_retries = 10
    gondola_y = 0.0
    visible_count = 0
    hazard = True
    
    # Use AABB zmax as the upper bound for gondola height, with a reasonable fallback
    ...
    if forced_height is not None or forced_workers is not None:
        ...
        visible_count = (
            _set_workers_visible_count(active_diaolan, forced_workers)
            if forced_workers is not None
            else randomize_workers(stage, active_diaolan, rng)
        )
        ...
        for attempt in range(max_retries):
            ...
            # Randomize workers
            visible_count = randomize_workers(stage, active_diaolan, rng)
    ...
    print(f"[diaolan-workers] persons_visible={visible_count} group1_z={gondola_y:.2f} is_on_ground={gondola_y < 0.12 + 0.5} hazard={hazard}")

    # Apply height to group1 and sync workers
    with _scene_lock:
    ...
    if not sampling_on:
        ...
        preset_vis = check_preset_visibility(
            stage, _CAMERA_RIG_PRIM, presets_cfg, _GONDOLA_PRIM
        )
        pv_line = _preset_visibility_log_line(presets_cfg, preset_vis)
        ok = bool(any(preset_vis.values()))
        print(
        ...
        for attempt in range(max_cam_retries):
            ...
            cam_x, cam_y, cam_z, _, box_meta = sample_camera_in_changjing(
                ...
                seed,
                sample_box=_box,
                wall_prim_path=_CAMERA_HEIGHT_WALL_PRIM,
            )
            last_seed = seed
    ...
    def do_POST(self):
        if self.path == "/scene/gondola":
            ...
            try:
                ...
                with _scene_lock:
                    if "y" in req:
                        _scene_state["gondola_y"] = max(
                            _GONDOLA_HEIGHT_MIN,
                            min(_GONDOLA_HEIGHT_MAX, float(req["y"])),
                        )
                _scene_dirty.set()
                with _scene_lock:
...
def main():
    ...
    if rtsp_enabled and _ffmpeg_proc is not None:
        ...
        ff_ok = False
    _stream_diag_update(ffmpeg_alive=ff_ok, mediamtx_started=_mediamtx_proc is not None)
