...
└── CameraRig              [Xform]  PTZ 相机控制根节点
    ...
    │   xformOp:rotateZ    = Pan 水平旋转（-90° ~ +90°）
    └── CamTilt            [Xform]
        │   xformOp:rotateY = Tilt 俯仰旋转（-90° ~ +90°）
...
| 控件 | 功能 | 范围 |
|------|------|------|
| 2D 摇杆 | 同时控制水平角（Pan）和俯仰角（Tilt） | Pan: -90°~+90°，Tilt: -90°~+90° |
| 变焦滑条 | 光学变焦倍数 | 1× ~ 32× |
| 预设按钮 | 快捷跳转到预设角度 | — |
...
| GET | `/ptz/selfcheck` | 返回 PTZ 零位、方向符号、期望写入值与当前场景实际旋转值（方向自检） |
| GET | `/runtime/model` | 查询当前“运行时”场景配置（scene_path/camera_prim） |
| POST | `/runtime/model` | 更新运行时场景配置，可选运行中自动重启应用新场景 |

`POST /runtime/model` 请求示例：
```http
POST /runtime/model
...
|-----------|------|--------------|------|
| pan（x） | \[-1, 1\] | pan_deg | `× 90.0` → \[-90°, 90°\] |
| tilt（y） | \[-1, 1\] | tilt_deg | `× 90.0` → \[-90°, 90°\] |
| zoom（x） | \[0, 1\] | zoom_x | `1.0 + × 31.0` → \[1×, 32×\] |

预置位（GotoPreset token）：`"1"` = 当前镜头基准位、`"2"` = 右转 90°、`"3"` = 左转 90°、`"home"` = 默认位。
