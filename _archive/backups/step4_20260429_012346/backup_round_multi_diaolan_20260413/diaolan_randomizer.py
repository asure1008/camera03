...
def sync_workers_to_group1(stage, group1_path, person_paths, active_count):
    ...
    # We need to copy the height-axis translation from Group1 to persons.
    ...
    for p_path in person_paths:
        ...
        x, y, z = float(current_val[0]), float(current_val[1]), float(current_val[2])
        if h_idx == 2:
            trans_op.Set(Gf.Vec3d(x, y, h_val))
        else:
            trans_op.Set(Gf.Vec3d(x, h_val, z))
...
def _collect_wall_mount_candidates(stage, wall_path, wall_collection_mode=None, wall_collection_root_path=None):
    ...
    if mode in ("explicit_root_local", "global_wall_candidates"):
    ...
    if not candidates:
        if mode == "global_wall_candidates":
            reason = (
                f"global_wall_candidates_empty:root={collection_root.GetPath().pathString} "
                f"total_scanned={total_scanned} valid={valid_filter_count} after_mode={mode_filter_count}"
            )
            print(f"[wall-constraint] ERROR: {reason}")
...
def _select_wall_mount_reference(stage, wall_path, rng, wall_collection_mode=None, wall_collection_root_path=None):
    ...
    print(
        ...
        + "; ".join(_format_mount_candidate(c) for c in candidates)
    )
    print(
        ...
        f"common_height_range={[round(float(wall_pool['common_height_min']),4), round(float(wall_pool['common_height_max']),4)] if wall_pool['common_height_min'] is not None and wall_pool['common_height_max'] is not None else None}"
    )
    if not candidates:
        raise RuntimeError(f"failed to resolve wall mount reference from {wall_path}")
