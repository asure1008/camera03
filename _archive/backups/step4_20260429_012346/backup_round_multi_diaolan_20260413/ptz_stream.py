...
_RENDERER_TARGET_MODE = "PathTracing"
_WALL_CONSTRAINT_PRIM_DEFAULT = "/World/Changjing_Jichu_2026_04_01/Changjing_AlgorithmVerification_2026_03/Model/Group5/Group7/AIShaoBing_Wall_Metal_2_26/Component431_26/Mesh228"
...
def _enforce_renderer_mode(reason: str, reset_launch_config: bool = False) -> None:
    ...
    sim_app.set_setting("/rtx/rendermode", renderer_mode)
    sim_app.update()
    import carb
    ...
    print(
        "[renderer-fix] "
        f"reason={reason} target={renderer_mode} "
        f"/app/renderer/plugin={renderer_plugin} "
        f"/rtx/rendermode={render_mode}"
    )
    if str(render_mode).strip().lower() != renderer_mode.lower():
        raise RuntimeError(
            f"renderer enforcement failed: expected {renderer_mode}, got {render_mode}"
        )
...
def _compose_status_dict() -> dict:
    stage = omni.usd.get_context().get_stage()
    with _ptz_lock:
        out = dict(_ptz_state)
    with _stream_diag_lock:
        out["stream"] = dict(_STREAM_DIAG)
    out["scene"] = _scene_state_snapshot(stage)
...
_scene_state = {
    ...
    "random_config": {
        ...
        "auto_random_timer_enabled": False,
...
_RANDOMIZE_REQ_META_KEYS = frozenset(
    {"trigger", "active_diaolan_path", "runtime_random_config", "is_auto", "source", "request_id"}
)
...
def _scene_state_runtime_snapshot() -> dict:
    with _scene_lock:
        return {
        ...
        lights.append(
            {
                ...
                "effective_for_render": bool(is_active and str(visibility) != "invisible"),
            }
        )
...
def _refresh_hdri_backend_status_from_stage(stage, control_state: dict | None = None, updated_at: str | None = None) -> dict:
    ...
    return _store_hdri_backend_status(
        {
            "current_hdri_name": hdri_state.get("current_hdri_basename"),
            "current_hdri_path": hdri_state.get("current_hdri"),
...
def _scene_state_snapshot(stage=None) -> dict:
    stage = stage or omni.usd.get_context().get_stage()
    ...
    current_target_path = _GONDOLA_PRIM or None
    all_paths_snap = list(runtime.get("all_diaolan_paths") or [])
    if not all_paths_snap:
        all_paths_snap = [str(d.get("path") or "").strip() for d in scanned if str(d.get("path") or "").strip()]
...
def _apply_hdri_control_request(stage, req: dict | None = None, route: str = "/scene/hdri") -> dict:
    ...
    try:
        ...
        apply_ok = False
        apply_message = ""
        if isinstance(target_entry, dict) and target_entry.get("exists") and target_entry.get("path"):
...
def _api_scene_randomize_last_dict() -> dict:
    with _scene_lock:
        last = _scene_state.get("last_random_result")
...
_snapshot_cache = {"jpeg": None, "frame_id": 0, "ts": 0.0}
...
def _http_get_status_body_cached() -> bytes:
    ...
    body = json.dumps(_compose_status_dict(), ensure_ascii=False).encode("utf-8")
    with _STATUS_HTTP_CACHE_LOCK:
...
def _http_get_scene_state_body_cached() -> bytes:
    ...
    with _SCENE_STATE_HTTP_CACHE_LOCK:
        if (
            _scene_state_http_cache_body is not None
            and (now - _scene_state_http_cache_ts) < _SCENE_STATE_HTTP_CACHE_TTL_S
        ):
            return _scene_state_http_cache_body
...
def _apply_dynamic_lookat_after_random_camera(stage, camera_world_xyz, *, source: str = "random_camera", preset_name: str = "default_initial", apply_to_stage: bool = True, target_xyz=None) -> tuple[float, float] | None:
    ...
    try:
        target_xyz = resolved_target_xyz
        base_pan, base_tilt = _compute_dynamic_lookat_pan_tilt(camera_world_xyz, target_xyz)
    ...
    if (
        str(source or "").strip().lower() == "random_camera"
    ...
    tilt_deg = max(-90.0, min(30.0, float(tilt_deg)))
...
def _compute_target_projection_metrics(stage) -> dict | None:
    """计算当前 target 在真实运行 Stage 上的投影指标，供验收脚本直接读取。"""
    try:
        ...
        tgt_prim = stage.GetPrimAtPath(_GONDOLA_PRIM)
...
def _refresh_projection_metrics(stage) -> None:
    _stream_diag_update(target_projection_metrics=_compute_target_projection_metrics(stage))
...
def _choose_visible_worker_paths(person_paths: list[str], visible_count: int, rng=None) -> list[str]:
    normalized = [str(p) for p in person_paths if str(p).strip()]
    if not normalized:
        return []
...
def _set_active_worker_paths(active_diaolan_path: str, person_paths: list[str], visible_paths: list[str]) -> None:
    allowed = [str(p) for p in person_paths if str(p).strip()]
    allowed_set = set(allowed)
    visible = [str(p) for p in visible_paths if str(p).strip() and str(p) in allowed_set]
...
def _apply_active_worker_visibility(stage) -> int:
    ...
    for worker_path in all_paths:
        _set_prim_visibility(stage, worker_path, False)
    visible_count = 0
    for worker_path in all_paths:
        should_show = worker_path in visible_paths
        _set_prim_visibility(stage, worker_path, should_show)
        if should_show:
            visible_count += 1
    return visible_count


def _iter_active_gondola_renderables(stage, active_diaolan_path: str, worker_paths: list[str]):
    from pxr import Usd as _Usd
    from pxr import UsdGeom as _UsdGeom
    ...
    worker_roots = [str(p).rstrip("/") for p in worker_paths if str(p).strip()]
    renderables = []
    for prim in _Usd.PrimRange(root_prim):
        prim_path = prim.GetPath().pathString
        if any(prim_path == root or prim_path.startswith(root + "/") for root in worker_roots):
            continue
...
def _repair_active_gondola_renderables(stage) -> dict:
    from pxr import Usd as _Usd
    from pxr import UsdGeom as _UsdGeom
    ...
    renderables = _iter_active_gondola_renderables(stage, active_diaolan_path, worker_paths)
...
def _set_workers_visible_count(diaolan_info: dict, visible_count: int, *, rng=None) -> int:
    ...
    person_paths = list(diaolan_info.get("persons") or [])
    visible_paths = _choose_visible_worker_paths(person_paths, visible_count, rng=rng)
    _set_active_worker_paths(diaolan_info.get("path", ""), person_paths, visible_paths)
    stage = omni.usd.get_context().get_stage()
    return _apply_active_worker_visibility(stage) if stage is not None else len(visible_paths)


def _apply_scene_state(stage) -> None:
    """运行时同步吊篮高度、工人显隐与吊篮可渲染节点可见性。"""
    _apply_gondola_height_only(stage)
    _apply_active_worker_visibility(stage)
    _repair_active_gondola_renderables(stage)
...
def _resolve_startup_lookat_target_xyz(stage, target_prim_path: str | None) -> tuple[float, float, float]:
    target_xyz = tuple(float(v) for v in _CAMERA_LOOKAT_TARGET_XYZ)
    print(
        "[camera-startup-target] "
        f"source=config_camera_lookat_target_xyz target_xyz={tuple(round(float(v), 4) for v in target_xyz)}"
    )
    return target_xyz
...
def _apply_active_diaolan_switch(
    ...
    *,
    ...
    _set_active_worker_paths(path, active_diaolan.get("persons") or [], [])
    return hide_result
...
def _randomize_scene_runtime(request_meta: dict | None = None) -> dict:
    ...
    active_diaolan = pick_active_diaolan(
        all_diaolans,
        rng,
        forced_path=forced_path if forced_path else None,
    )
    ...
    previous_workers = int(state_before["workers"])
    if random_cfg["random_workers"]:
        requested_worker_count = rng.randint(0, len(active_diaolan.get("persons") or []))
        visible_paths = _choose_visible_worker_paths(active_diaolan.get("persons") or [], requested_worker_count, rng=rng)
    else:
        previous_visible = list(state_before.get("visible_worker_paths") or [])
        if (
            state_before.get("active_diaolan_path") == active_diaolan["path"]
            and previous_visible
            and all(path in (active_diaolan.get("persons") or []) for path in previous_visible)
        ):
            visible_paths = previous_visible[: max(0, min(len(active_diaolan.get("persons") or []), previous_workers))]
        else:
            visible_paths = _choose_visible_worker_paths(active_diaolan.get("persons") or [], previous_workers)
    visible_count = len(visible_paths)
    _set_active_worker_paths(active_diaolan["path"], active_diaolan.get("persons") or [], visible_paths)

    with _scene_lock:
        _scene_state["gondola_y"] = gondola_y
        _scene_state["gondola_heights"] = dict(gondola_heights)
        _scene_state["workers"] = max(0, min(2, int(visible_count)))
    ...
    target_xyz = _resolve_startup_lookat_target_xyz(stage, _GONDOLA_PRIM)
    camera_meta = {
        "mode": "unchanged",
        ...
        for attempt in range(max_cam_retries):
            ...
            if random_cfg["keep_wall_install_constraint"]:
                cam_x, cam_y, cam_z, _, camera_meta = sample_camera_in_changjing(
                    stage,
                    "/World/Changjing_AlgorithmVerification_2026_03",
                    _CAMERA_RIG_PRIM,
                    rng,
                    seed,
    ...
    result = {
        "timestamp": time.strftime("%Y-%m-%d %H:%M:%S"),
        "diaolan_candidates": [str(d.get("path") or "") for d in all_diaolans],
        "requested_active_diaolan_path": forced_path or None,
        "forced_active_diaolan_path": forced_path or None,
        ...
        "workers": int(scene_after["workers"]),
        "visible_workers": len(scene_after["visible_worker_paths"]),
        "workers_count": len(scene_after["visible_worker_paths"]),
        "workers_total_available": len(active_diaolan.get("persons") or []),
    ...
    _annotate_randomize_result_for_api(result, request_meta)
    with _scene_lock:
...
def _init_gondola_paths_camera_and_random_height_once(stage) -> None:
    ...
    _apply_scene_state(stage)
    sync_workers_to_group1(stage, _GONDOLA_PRIM, active_diaolan["persons"], visible_count)
    ...
    startup_lookat_target_xyz = _resolve_startup_lookat_target_xyz(stage, _GONDOLA_PRIM)
...
class _PTZHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == "/ptz_state":
            with _ptz_lock:
                snap = {"pan": float(_ptz_state["pan"]), "tilt": float(_ptz_state["tilt"]), "zoom": float(_ptz_state["zoom"])}
        ...
        if self.path == "/scene/random-config":
            body = json.dumps(_scene_state_runtime_snapshot()["random_config"], ensure_ascii=False).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self._cors()
            self.end_headers()
        ...
        if self.path == "/api/scene/random-config":
            with _scene_lock:
                rc = _sanitize_random_config(_scene_state.get("random_config"))
            body = json.dumps({"ok": True, "random_config": rc}, ensure_ascii=False).encode("utf-8")
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            self._cors()
            self.end_headers()
    ...
    def do_POST(self):
        ...
        if self.path == "/scene/workers":
            ...
            try:
                ...
                with _scene_lock:
                    if "count" in req:
                        requested_count = max(0, min(2, int(req["count"])))
                        _scene_state["workers"] = requested_count
                        all_worker_paths = list(_scene_state.get("all_worker_paths") or [])
                        current_visible_paths = list(_scene_state.get("visible_worker_paths") or [])
                        if (
                            requested_count == len(current_visible_paths)
                            and all(path in all_worker_paths for path in current_visible_paths)
                        ):
                            next_visible_paths = current_visible_paths
                        else:
                            next_visible_paths = _choose_visible_worker_paths(all_worker_paths, requested_count)
                        _scene_state["visible_worker_paths"] = list(next_visible_paths)
                _scene_dirty.set()
                _invalidate_scene_state_http_cache()
                resp = json.dumps({"ok": True, "state": _scene_state_snapshot()}, ensure_ascii=False)
            except Exception as exc:
                resp = json.dumps({"ok": False, "error": str(exc)}, ensure_ascii=False)
            self.send_response(200)
            self.send_header("Content-Type", "application/json; charset=utf-8")
            ...
            try:
                ...
                if action == "describe":
                    ...
                    resp_obj = {
                        "ok": True,
                        "data": {
                            **_describe_active_scene_branch(stage),
                            "apply_result": apply_result,
                        },
                    }
...
def main():
    global _ffmpeg_proc, _mediamtx_proc, _jpeg_encode_fn, _c_local_frame_saved
    ...
    if _rig_world_xyz is not None:
        dyn_result = _apply_dynamic_lookat_after_random_camera(
            ...
            target_xyz=_resolve_startup_lookat_target_xyz(st0, _GONDOLA_PRIM),
        )
    ...
    while _running and sim_app.is_running():
        ...
        if _scene_randomize_dirty.is_set():
            ...
            if isinstance(pending_randomize, dict):
                ...
                try:
                    randomize_result = _randomize_scene_runtime(request_meta)
            ...
            if rgba is not None and rgba.size > 0:
                ...
                if _jpeg_encode_fn is not None:
                    ...
                    if not need_snapshot:
                        ...
                        need_snapshot = snapshot_empty or (now_snapshot - last_snapshot_ts >= snapshot_interval_s)
                    if need_snapshot:
                        t0 = time.monotonic()
