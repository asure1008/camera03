...
def _select_diaolan_response_state(stage) -> dict:
    ...
    for p in paths:
        ...
        if not pv:
            continue
        g1 = f"{pv.rstrip('/')}/Model/Diaolan_01"
        diaolan_candidates.append(
            {
                "path": pv,
                "target_prim_path": g1,
...
def _apply_select_diaolan_http(path: str) -> dict:
    ...
    try:
        ...
        root = path.rstrip("/")
        group1 = f"{root}/Model/Diaolan_01"
        persons = [f"{group1}/People_01", f"{group1}/People_02", f"{group1}/People_03"]
        minimal_d = {"path": path, "group1": group1, "persons": persons}
        _GONDOLA_PRIM = group1
...
def _http_json_scene_randomize(body: bytes, *, route_source: str, api_envelope: bool) -> str:
    ...
    try:
        ...
        if resp_data.get("ok") and isinstance(resp_data.get("result"), dict):
            resp_data["hazard_eval"] = resp_data["result"].get("hazard_eval")
        if api_envelope:
...
def _set_active_worker_paths(active_diaolan_path: str, person_paths: list[str], visible_paths: list[str]) -> None:
    ...
    with _scene_lock:
        ...
        if ap:
            wb[ap] = max(0, min(len(allowed), len(visible)))
...
def _apply_scene_state(stage) -> None:
    ...
    _apply_all_diaolans_worker_visibility(stage)
...
def _resolve_active_worker_count_for_hazard(result: dict) -> tuple[int | None, str | None]:
    ...
    if isinstance(visible_paths, list):
        return len(visible_paths), "visible_worker_paths"
    return None, None
...
def _service_stream_frame_during_randomize(annotator, *, stage_label: str) -> None:
        ...
        if ffmpeg_alive:
            ...
            try:
                ...
                try:
                    ...
                    queue_status = "queue_still_full"
        print(
            f"[scene-randomize][frame-service] stage={stage_label} "
...
def _randomize_scene_runtime(request_meta: dict | None = None, annotator=None) -> dict:
    ...
    _service_stream_frame_during_randomize(annotator, stage_label="before_result_snapshot")
